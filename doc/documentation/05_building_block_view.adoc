...
| _Die Informationen über die Dateien holt er sich vom `FileInfoService`. Um die Berechtigungen eines einzelnen Benutzers zu überprüfen, wird eine Anfrage an den `GroupService` & `SecurityService` gestellt, während die Berechtigungen des Ordners vom `PermissionService` zur Verfügung gestellt werden. Um Operatioen, die andere Ordner involvieren, ausführen zu können, wird beim `DirectoryRepository` angefragt._
...
| _Er vergleicht die Berechtigungen, die aus dem `PermissionService` kommen mit den Rollen des Users, die aus dem `GroupService` kommen._
|===
