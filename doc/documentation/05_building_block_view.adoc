...
----
Die erste Zeile erlaubt Up-/Download von Dateien.
Die `max-file-size` ist die maximale Uploadgröße und die `max-request-size` ist die maximale Downloadgröße.
Zweitere darf nur erhöht werden, da sonst alte Dateien nicht herunter geladen werden können. `max-swallow-size` nach
unserer Recherche wird sonst nicht die maximale Filegrößenfehler, die von uns implementiert sind, nicht geworfen.
...
|Komponente |Funktion | Schnittstellen
| DirectoryService
| _Stellt sämtliche Funktionen zur Verwaltung eines Ordners zur Verfügung. Das beinhaltet das Erstellen, Löschen und
Durchsuchen von Ordnern, das wechseln zu einem Unterordner sowie das Hochladen einer Datei in den Ordner._
| _Die Informationen über die Dateien werden vom `FileInfoService` abgerufen. Die Berechtigungen des Ordners werden vom
`PermissionService` zur Verfügung gestellt und können verändert werden.
Die vom `GroupService` bereitgestellten Rollen werden vom `SecurityService` überprüft, damit ein User nur erlaubte
Aktionen ausführen kann. Für das Speichern und Abrufen der Ordner wird mit dem `DirectoryRepository` interagiert._
...
| _Mittels einer Anfrage an den `FileInfoService` werden die Metadaten angefragt. Mit diesen wiederum kann eine Anfrage
an das `FileRepository` gestellt werden, um die Datei selbst zu erhalten. Berechtigungen werden mittels des
`DirectoryService` und des `SecurityService` überprüft._

| GruppenfindungsService
| _Erlaubt es Anfragen an die `Gruppenfindung` zu stellen._
| _Kommunziert nur über eine REST-API mit der `Gruppenfindung`._
...
| _Stellt den Zugang zu der gespeicherten `latestEventId` zur Verfügung. Wird für die Gruppenfindung benötigt._
...
| GroupRepository
| _Ist die Schnittstelle zur `Datenbank` für die Gruppen._
| _Stellt eine Anfrage an die `Datenbank`, um ein Gruppenobjekt zu erhalten._

| LatestEventIdRepository
| _Ist die Schnittstelle zur `Datenbank` für die `latestEventId`._
| _Stellt eine Anfrage an die `Datenbank`, um das `latestEventId`-Objekt zu erhalten. Es wird immer nur höchstens eins geben._
...
|===
| Komponente | Funktion | Schnittstellen
| DirectoryController | _Nimmt Anfragen mit der Routen beginnend mit `dir` entgegen._ |
...
include::../api/generated-snippets/index/DirectoryController/zip-download-test/path-parameters.adoc[]
...
include::../api/generated-snippets/index/FileController/get-file/path-parameters.adoc[]
