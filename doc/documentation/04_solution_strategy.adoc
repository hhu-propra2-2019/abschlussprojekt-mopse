...
Wir haben uns nach einer kurzen Diskussion für PostgreSQL entschieden.
Der Großteil unserer Erfahrungen beruhte bisher zwar auf MySQL, aber wir haben uns trotzdem für PostgreSQL entschieden,
weil es von den Server-Admins der HHU bevorzugt wird..

Leider dauert das Starten der PostgreSQL-Datenbank in einem Docker-Container recht lange (ähnlich wie MySQL) und es ist umständlich immer diesen Docker-Container laufen zu lassen.
